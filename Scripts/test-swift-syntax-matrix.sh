...
for version in "${versions[@]}"; do
  ...
  case "$matrix_mode" in
    xcode27)
      swift build --build-system swiftbuild --scratch-path "$scratch_path/swiftbuild"
      swift test --build-system swiftbuild --scratch-path "$scratch_path/swiftbuild"
      ;;
    hosted)
      swift build --scratch-path "$scratch_path/hosted-default"
      swift test --scratch-path "$scratch_path/hosted-default"
      ;;
    *)
  ...
  swift build --build-system native --scratch-path "$scratch_path/native" -Xswiftc -warnings-as-errors
  swift test --build-system native --scratch-path "$scratch_path/native" -Xswiftc -warnings-as-errors
done
