...
|---|---|---|---|
| generate | accessor pairs over stored properties | 0.64--0.66x | **0.75--0.79x** |
| case-factory | static factories over enum cases | 0.57--0.58x | **1.00--1.06x** |
| case-path | `@CasePathable`-style `AnyCasePath` property per case | 0.47--0.48x | **0.73--0.78x** |
...
- **Trivia: measured at 1--4%, corrected on one workload.** The baselines used to attach no trivia at all, emitting `staticfuncmakeCase0(_value:String)` where the library emits spaced source. The equivalence gate compares token streams, which strips trivia, so it could not see this. The `case-factory` baselines now match the library byte for byte, and a control pipeline (`interned-notrivia`) keeps the old construction so the difference can be measured in the same process: it is worth 1% at 16 cases and 3.8% at 256. `generate` and `case-path` baselines still omit trivia, so those two rows are conservative by a now-bounded ~1--4%. Inspect with `--workloads trivia`.
- **Typing the pattern cost 10%, and the table reflects it.** The guard-case pattern in `case-path` (`case let .foo(value) = $0`) used to ride through `.variable`'s raw-source escape hatch, which measured 0.67--0.68x. Rewriting it with `MatchPattern` -- an indirect enum, so a box and a subpattern array per case where the string version allocated one interpolated `String` -- moved it to 0.74--0.77x. The typed version is what ships and what is measured, because a benchmark that reaches for the escape hatch is measuring a library you would not actually write with. If you need the last 10% on a hot pattern, `.variable` is still there.
