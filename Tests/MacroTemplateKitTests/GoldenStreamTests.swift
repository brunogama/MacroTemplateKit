...
enum ParityGoldens {
    ...
    static let goldenStatementStreams: [String] = [
        ...
        "throw MyError ( )",
        "var y = 2",
        "guard case 200 = status else { return }",
        "if case 201 = status { break } else { break }",
    ]
...
final class GoldenStreamTests: XCTestCase {
    func testCorpusCoversEveryTemplateAndStatementCase() {
        XCTAssertEqual(
            Set(ParityCorpus.templates.map { ParityCorpus.templateCase(of: $0) }),
            Set(ParityCorpus.TemplateCase.allCases)
        )
        XCTAssertEqual(
            Set(ParityCorpus.statements.map { ParityCorpus.statementCase(of: $0) }),
            Set(ParityCorpus.StatementCase.allCases)
        )
    }
    ...
    func testTemplateGoldenStreams() throws {
        ...
        for (template, golden) in zip(ParityCorpus.templates, ParityGoldens.goldenTemplateStreams) {
            XCTAssertEqual(try tokenStream(Renderer.render(template)), golden)
        }
    }
    ...
    func testStatementGoldenStreams() throws {
        ...
        for (statement, golden) in zip(ParityCorpus.statements, ParityGoldens.goldenStatementStreams) {
            XCTAssertEqual(try tokenStream(Renderer.render(statement)), golden)
        }
    }
    ...
    func testDeclarationGoldenStreams() throws {
        ...
        for (declaration, golden) in zip(
            ParityCorpus.declarations, ParityGoldens.goldenDeclarationStreams
        ) {
            XCTAssertEqual(try tokenStream(Renderer.render(declaration)), golden)
        }
    }
