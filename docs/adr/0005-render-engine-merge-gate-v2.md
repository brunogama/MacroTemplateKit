...
- **The baseline was beatable.** [ADR 0004](0004-baselines-must-hoist-invariants.md) found the
  `structural` pipeline rebuilding loop invariants; hoisting alone made it up to 1.75× faster.
...
- **One workload is not enough.** `generate` was the only gated shape. `case-factory` sits at
  parity and `case-path` at 0.68×; a gate on `generate` alone would have reported none of that.
...
1. **`mtk` beats `structural-interned` on `generate` and `case-path`,** and is within 5% of
   it on `case-factory`, where it currently sits at 1.00–1.02× — i.e. marginally slower. The
...
| workload | 16 | 64 | 256 |
|---|---|---|---|
| generate | 0.74× | 0.74× | 0.74× |
| case-factory | 1.00× | 1.02× | 1.00× |
| case-path | 0.69× | 0.67× | 0.68× |
...
- The trivia bias flagged in ADR 0004 is now corrected on every baseline. It was worth 1–4% on
  `case-factory` and ~8% on `case-path`, so the figures above are better than the previous
  revision's. A correction in our own favour deserves the same scrutiny as one against, which
  is why `interned-notrivia` stays in the registry as a control.
